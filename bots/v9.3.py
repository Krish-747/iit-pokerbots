...
OPP_RAISE_THRESH  = [0.20, 0.28, 0.42, 0.58, 0.72]  # FIX: was [0.35,0.45,0.60,0.75,0.85] → 82% fold rate to tiny bets
BAYESIAN_DISCOUNT = 0.88
BAYESIAN_FLOOR    = 0.78   # FIX: cap compounding discount — was unlimited, e.g. 0.88^3=0.68
...
class Player(BaseBot):
            ...
            if street == 'pre-flop':
                ...
                if delta > max(cs.pot * 0.15, 15):
                    ...
                    if self.opp.pf_raise_rate <= 0.45:
                        ...
                        self._wr_discount = max(BAYESIAN_FLOOR, self._wr_discount)  # FIX: floor
                else:
    ...
    def _auction_bid(self, cs):
        ...
        # FIX: old cap was min(110, pot*0.45) = 9 chips on pot=20.
        # chippy_chips bids 19-25 and won 79% of auctions because our cap was lower.
        # Use a flat minimum of 22 so we can always compete on small pots.
        abs_cap = max(22, min(110, int(pot * 0.55)))
        raw   = max(floor, min(target, chip_cap, abs_cap))
    ...
    def on_hand_start(self, game_info: GameInfo, cs: PokerState) -> None:
        ...
        self._auction_pre_pot  = 0
        self._our_last_bid     = 0
        self._opp_won_auction  = False   # FIX: track when opp knows our card
        self._current_street   = 'preflop'
        self._was_pf_aggressor = False
    ...
    def get_move(self, game_info: GameInfo, cs: PokerState):
        ...
        if street == 'flop' and self._auction_pre_pot > 0:
            ...
            # FIX: cs.opp_revealed_cards holds the revealed card regardless of winner.
            # If revealed card is in OUR hand → OPP WON (our card was shown to them).
            # If revealed card is NOT in our hand → WE WON (their card shown to us).
            if opp_known and opp_known not in cs.my_hand:
                # We won: record their exact bid
                if 0 < pot_delta < 5000:
                    self.opp.record_auction_win(int(pot_delta))
                self._opp_won_auction = False
            elif opp_known and opp_known in cs.my_hand:
                # Opp won: revealed card is our own — clear it, useless for MC
                opp_known = None
        ...
        if street == 'pre-flop':
            ...
            # FIX: extreme maniac detection (raises 250 every hand like Single_G).
            # When opp raises >5x BB on EVERY hand, our call range must widen or
            # we bleed blinds on 738 folds. Add separate is_extreme_maniac flag.
            is_extreme_maniac = self.opp.pf_raise_rate > 0.75
            ...
            if is_pair and hi >= 13:
                ...
                if raise_level == 0 and can(ActionRaise):
                    ...
                    # FIX: extreme maniacs raise huge every hand (75%+ PF raise rate).
                    # Chen>=7 is profitable to call vs their wide range.
                    if is_extreme_maniac and can(ActionCall) and pf_wr >= call_equity_needed:
                        return ActionCall()
                    if can(ActionFold):  return ActionFold()
                    if can(ActionCheck): return ActionCheck()
                return ActionFold()
                ...
                if raise_level == 0 and can(ActionRaise):
                    ...
                    return ActionCall()
                # FIX: pairs 55-66 (chen=5-6) call extreme maniacs at raise_level=2
                elif raise_level == 2 and is_pair and is_extreme_maniac and can(ActionCall) and pf_wr >= call_equity_needed:
                    return ActionCall()
                if can(ActionCheck): return ActionCheck()
                return ActionFold()
        ...
        if street in ['flop', 'turn', 'river']:
            ...
            # FIX: When opp won auction they know our card and only bet when they have us.
            # 93% win rate confirmed (244 hands). Never raise into them; fold to any bet
            # unless holding absolute nuts (SF/quads/full_house).
            if self._opp_won_auction and cost > 0:
                if hclass in ('straight_flush', 'quads', 'full_house') and wr > 0.65:
                    if can(ActionCall): return ActionCall()
                elif hclass in ('flush', 'straight', 'set') and opp_r == 0 and wr > 0.60:
                    if can(ActionCall): return ActionCall()
                else:
                    if can(ActionFold): return ActionFold()
                    if can(ActionCheck): return ActionCheck()
            if self._opp_won_auction and cost == 0:
                # They checked — safe to value bet only nuts, otherwise check behind
                if hclass in ('straight_flush', 'quads', 'full_house') and can(ActionRaise):
                    self._street_raises[street] += 1
                    return ActionRaise(self._bet_size(cs, wr))
                if can(ActionCheck): return ActionCheck()
                return ActionFold()

            # FIX: downgrade straight to STRONG when flush is possible on board
            # R547: alpha had straight, opp had nut flush → raised 3x on river → -3656
            if hclass == 'straight' and texture in ('wet_flush',):
                hclass = 'two_pair'  # treat as STRONG, not MONSTER
            ...
            if is_allin:
                ...
                if hclass in MONSTER and wr > allin_req:
                    if can(ActionCall): return ActionCall()
                elif hclass in STRONG and wr > max(allin_req, 0.55):
                    # FIX: was max(allin_req, 0.50) — but wr>0.70 in normal flow also
                    # calls without checking allin_req. R860: alpha QsAs vs AA,
                    # wr~0.72 (opp_known=Ad only), allin_req=0.716 → should fold.
                    # Raising allin threshold to 0.55 AND enforcing allin_req here.
                    if can(ActionCall): return ActionCall()
                elif hclass in DECENT and wr > max(allin_req, 0.65):
                    if can(ActionCall): return ActionCall()
                if can(ActionFold):  return ActionFold()
                if can(ActionCheck): return ActionCheck()
