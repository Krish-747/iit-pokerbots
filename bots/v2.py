...
class Player(BaseBot):
    ...
    def _track_opp(self, current_state, street):
        if self._current_street != street:
            ...
            self._prev_opp_wager = 0
            self._wr_discount = 1.0  # Reset discount per street
    ...
    def _auction_bid(self, current_state):
        ...
        # Safety caps: drastically lower to prevent bleeding vs spam bidders
        ABS_CAP = 90
        FRAC_CAP = int(chips * 0.12)
        cap = min(ABS_CAP, FRAC_CAP)

        if wr_base >= 0.75:
            # Nuts: bid just enough to contest, but respect hard caps
            target = max(MIN_BID_FLOOR, min(p75_bid * 1.05, cap))

        elif 0.55 <= wr_base < 0.75:
            # Marginal: some value, but don't get trapped in a bidding war
            target = min(max(MIN_BID_FLOOR, avg_bid * 0.85), cap)

        else:
            # Weak/Below-average: save chips, don't buy useless info
            target = random.randint(2, 15)
