...
class Player(BaseBot):
    ...
    def _auction_bid(self, current_state):
        ...
        if wr_base > 0.80:
            # Very strong hand: bid aggressively to win the info
            target = int(pot * random.uniform(0.45, 0.70))
        elif wr_base > 0.60:
            # Strong hand: competitive bid to win more often than not
            target = int(pot * random.uniform(0.22, 0.45))
        elif wr_base > 0.45:
            # Medium hand: modest bid
            target = int(pot * random.uniform(0.10, 0.22))
        else:
            # Weak hand: small bid, don't bleed chips
            target = int(pot * random.uniform(0.04, 0.10))

        # Cap at 25% of remaining stack - no hard chip ceiling
        chip_cap = min(int(chips * 0.25), chips - 1, 4999)

        # Floor: never bid less than 10 - a 1-chip bid gifts info for free
        floor = max(10, int(pot * 0.04))

        return max(floor, min(target, chip_cap))
