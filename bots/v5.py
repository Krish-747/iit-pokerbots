'''
alpha_q bot v5.1 — Anti-Maniac & KQS-Analysis Rewrite

CRITICAL UPGRADES:
1. Dynamic PFR Defense: Detects if opp is a "maniac" (>45% raise rate) 
   and massively widens call_tolerance to punish blind 80-chip stealers.
2. Calibrated Auctions: Targets ~30% of pot (mirrors top bot behavior), 
   avoids over-bidding weak hands, uses hard safety caps.
3. C-Betting: If we raised pre-flop, we fire a 50-65% pot continuation 
   bet on the flop with decent hands or a 20% bluff frequency.
4. PFR boosted to ~42%: Chen 5+ raises first-in.
'''
...
class OpponentModel:
    ...
    @property
    def bid_75th(self):
        if len(self.opp_bids) < 8: return 40
...
class Player(BaseBot):
    ...
    def _cbet_size(self, cs):
        """Continuation bet: 50-65% pot"""
        mn, mx = cs.raise_bounds
        pot    = cs.pot
        chips  = cs.my_chips
        bet = int(pot * random.uniform(0.50, 0.65))
        bet = min(bet, int(chips * 0.40))
        return max(mn, min(bet, mx))
    ...
    def _auction_bid(self, current_state):
        ...
        # Weak hands save chips
        if wr_base < 0.30:
            return random.randint(3, 12)

        if self._hand_num <= SCOUTING_ROUNDS:
            if is_pair: target = random.uniform(40, 65)
            elif wr_base > 0.68: target = random.uniform(35, 55)
            elif wr_base > 0.50: target = random.uniform(50, 75)
            else: target = random.uniform(25, 45)
        ...
        if is_pair:
            ...
            target = max(pot_base, min(p75_bid * 1.05, pot * 0.38))
        elif 0.52 <= wr_base < 0.72:
            target = min(max(pot * 0.25, avg_bid * 0.85), pot * 0.32)
        else:
        ...
        return max(1, min(int(target), cap, chips-1, 4999))
    ...
    def get_move(self, game_info: GameInfo, current_state: PokerState):
        ...
        if street in ['flop', 'turn', 'river']:
            ...
            if street == 'river':
                raise_allowed = (raise_allowed and wr >= 0.72 and hclass in MONSTER | STRONG)
            ...
            if hclass in MONSTER:
                ...
                if raise_allowed and my_r == 0 and opp_r == 0:
                    self._street_raises[street] += 1
                    return ActionRaise(self._bet_size(current_state, wr))
                if can(ActionCall) and ev_call > 0 and cost < pot * 0.65:
                    return ActionCall()
                if can(ActionCheck): return ActionCheck()
                return ActionFold()
