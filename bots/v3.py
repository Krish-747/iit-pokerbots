'''
alpha_q bot v4 - Fixed from 17:22 game log analysis

ROOT CAUSE FIXES:
1. AUCTION BROKEN: pot*0.15 cap at pot=40 -> 6 chip bid (bot bid 6 on 94% of hands)
   BotA avg bid: 11.3  vs  BotB avg bid: 55.8  ->  BotA won only 19/327 auctions (5.8%)
   Fix: competitive floor of 50+ chips, scouting mode first 100 rounds
2. POST-FLOP: raising with weak hands (pair of 3s on JJ board, 77 on paired board)
   Fix: board-pair downgrades hand class, river raise requires 75%+ WR + strong hand
3. NO SCOUTING: wasting early rounds instead of gathering opponent data
   Fix: first 100 rounds bid flat 55-85 to calibrate opponent bidding patterns
4. OPP_BID TRACKING: was inferring from pot heuristic - now tracks directly
'''
...
SCOUTING_ROUNDS   = 100   # bid flat to learn opponent before optimizing
MIN_BID_FLOOR     = 50    # CRITICAL FIX: was pot*0.15 -> 6 chips at small pots
OPP_RAISE_THRESH  = [0.42, 0.52, 0.67, 0.80, 0.90]  # fold threshold by opp raise count
BAYESIAN_DISCOUNT = 0.82  # per opp raise, discount our winrate estimate
...
def classify_hand(hole_cards, board_cards):
    ...
    if counts[0] == 3:
        ...
        if hole_ranks[0] == trip_rank and hole_ranks[1] == trip_rank:
            return 'set'
        if trip_rank in hole_ranks:
            return 'trips'
        return 'board_trips'  # trips on board only - weak for us

    if counts[0] == 2 and len(counts) > 1 and counts[1] >= 2:
        pairs = [r for r,c in rc.items() if c == 2]
        hole_contributing = any(r in hole_ranks for r in pairs)
        if not hole_contributing:
            return 'board_two_pair'
        board_pair_count = sum(1 for r,c in brc.items() if c >= 2)
...
def board_texture(board_cards):
    ...
    if 14 in uniq:
        uniq.append(1)
    if any(uniq[i] - uniq[i+2] <= 3 for i in range(len(uniq)-2)):
        return 'wet_straight'
    if max(sc.values()) == 2:
        return 'drawy'
    return 'dry'
...
def mc_winrate(my_cards, board_cards, opp_known=None, iterations=300):
    ...
    if EVAL7_AVAILABLE:
        ...
        for _ in range(iterations):
            ...
            os = _eval7.evaluate(opp_hand + sim_board)
...
class OpponentModel:
    def __init__(self):
        self.opp_bids      = []   # directly observed opponent bids
    ...
    def avg_opp_bid(self):
        if not self.opp_bids: return 55
        return sum(self.opp_bids[-30:]) / len(self.opp_bids[-30:])

    @property
    def bid_75th(self):
        if len(self.opp_bids) < 8: return 80
        s = sorted(self.opp_bids)
        return s[int(len(s) * 0.75)]

    @property
    def is_aggressive(self):
        if self.postflop_total < 10: return False
        return (self.postflop_raises / max(1, self.postflop_total)) > 0.35

    @property
    def pf_fold_rate(self):
        return self.pf_folds / max(1, self.pf_total) if self.pf_total >= 10 else 0.30
...
class Player(BaseBot):
    ...
    def _bet_size(self, cs, wr):
        ...
        else:           frac = random.uniform(0.35, 0.55)
        bet = int(pot * frac)
        bet = min(bet, int(chips * 0.35))
        return max(mn, min(bet, mx))

    # -----------------------------------------------------------------------
    # AUCTION - THE CRITICAL FIX
    # -----------------------------------------------------------------------
    def _auction_bid(self, current_state):
        ...
        # TRASH: save chips entirely
        if wr_base < 0.28:
            return random.randint(2, 10)

        # SCOUTING MODE (first 100 rounds): bid flat to learn opponent
        if self._hand_num <= SCOUTING_ROUNDS:
            # Bid competitively but not recklessly during scouting
            # Scale slightly with hand strength
            if wr_base > 0.70:
                target = random.uniform(45, 70)
            elif wr_base > 0.50:
                target = random.uniform(60, 90)  # marginal = most info value
            else:
                target = random.uniform(40, 65)
            return max(1, min(int(target), chips-1, 4999))

        # CALIBRATED MODE (after scouting)
        avg_bid = self.opp.avg_opp_bid
        p75_bid = self.opp.bid_75th

        # Estimate information value
        known    = set(my_cards + board)
        possible = [c for c in FULL_DECK if c not in known]
        sample   = random.sample(possible, min(20, len(possible)))
        t0       = time.time()
        wrs      = [mc_winrate(my_cards, board, c, 50) for c in sample]
        self._total_time += time.time() - t0
        info_delta  = sum(abs(w - wr_base) for w in wrs) / max(1, len(wrs))
        true_value  = info_delta * pot * 3.0

        if wr_base > 0.82:
            # Nuts: info helps but don't overpay - bid just enough to often win
            target = avg_bid * random.uniform(0.70, 0.90)
            target = max(MIN_BID_FLOOR, target)

        elif 0.45 <= wr_base <= 0.70:
            # Marginal: MOST valuable to know opp card - bid aggressively
            target = true_value * 2.0 + random.uniform(20, 50)
            # Beat opp's 75th percentile to actually WIN this auction
            target = max(target, p75_bid * 1.10)
            target = max(MIN_BID_FLOOR, target)

        else:
            # Below-average but worth something (0.28-0.45)
            target = true_value * 1.5 + random.uniform(15, 35)
            target = max(target, avg_bid * 0.85)
            target = max(MIN_BID_FLOOR, target)

        # Safety cap: never bet > 25% of stack or 500 chips on info
        target = min(target, chips * 0.25, 500)
    ...
    def get_move(self, game_info: GameInfo, current_state: PokerState):
        ...
        # Capture opponent's bid when we see flop after winning auction
        if street == 'flop' and self._auction_pre_pot > 0:
            if opp_known:  # we won - we paid opp's bid = pot delta
        ...
        if street == 'pre-flop':
            ...
            # QQ, AK: raise/4-bet, call 3-bet
            elif (is_pair and hi == 12) or (hi == 14 and lo == 13):
                if raise_level <= 2 and can(ActionRaise):
                    mn, mx = current_state.raise_bounds
                    return ActionRaise(max(mn, min(int(pot*3), mx)))
                elif raise_level >= 3 and can(ActionCall): return ActionCall()
                if can(ActionCall): return ActionCall()

            # JJ, TT, AQ, AJs: raise, call 3-bet, fold 4-bet
            ...
            # Chen 7+: raise once, call one raise
            elif chen >= 7.0:
                if raise_level == 0 and can(ActionRaise):
                    mn, mx = current_state.raise_bounds
                    return ActionRaise(max(mn, min(int(pot*2.5), mx)))
                elif raise_level == 1 and can(ActionCall) and cost <= pot: return ActionCall()
                elif raise_level >= 2:
                    if can(ActionFold): return ActionFold()
                if can(ActionCheck): return ActionCheck()
                return ActionFold()

            # Chen 4+: complete SB, call small raises
            elif chen >= 4.0:
                if raise_level == 0:
                    if can(ActionCall) and cost <= 20: return ActionCall()
                    if can(ActionCheck): return ActionCheck()
                elif raise_level == 1 and can(ActionCall) and cost <= pot*0.5: return ActionCall()
                if can(ActionCheck): return ActionCheck()
                return ActionFold()

            # Chen 2.5+: complete SB only
            elif chen >= 2.5:
            ...
            if can(ActionCall) and cost <= pot: return ActionCall()
            if can(ActionCheck): return ActionCheck()
            return ActionFold()
        ...
        if street in ['flop', 'turn', 'river']:
            ...
            # ----------------------------------------------------------------
            # BOARD TEXTURE DOWNGRADES (fixes Round 851, Round 999)
            # ----------------------------------------------------------------
            if texture == 'paired':
                # On paired board, two pair involving board pair is weak
            ...
            # RIVER: only raise with monster/strong AND good WR (fixes Round 503, 851, 999)
            if street == 'river':
                raise_allowed = (
                    raise_allowed and
                    wr >= 0.75 and
                    hclass in MONSTER | STRONG
                )

            # Loss aversion: if bleeding chips this hand, tighten
            chips_lost = self._chips_lost(current_state)
            if chips_lost > 1200:
                if wr < 0.65 and cost > 0:
                    if can(ActionFold): return ActionFold()
                raise_allowed = raise_allowed and wr >= 0.72
            ...
            if hclass in MONSTER:
                ...
                if raise_allowed and opp_r == 0:
                    self._street_raises[street] += 1
                    return ActionRaise(self._bet_size(current_state, wr))
                if can(ActionCall) and ev_call > 0 and cost < pot * 0.8:
                    return ActionCall()
                if can(ActionCheck): return ActionCheck()
                return ActionFold()
                ...
                if raise_allowed and my_r == 0 and opp_r == 0:
                    self._street_raises[street] += 1
                    return ActionRaise(self._bet_size(current_state, wr))
                if can(ActionCall) and ev_call > 0 and cost < pot * 0.6:
                    return ActionCall()
                if can(ActionCheck): return ActionCheck()
                return ActionFold()
                ...
                if raise_allowed and street == 'turn' and opp_r == 0 and wr > 0.50:
                    self._street_raises[street] += 1
                    return ActionRaise(self._bet_size(current_state, wr))
                if can(ActionCall) and ev_call > 0 and cost < pot * 0.70:
                    return ActionCall()
                if can(ActionCheck): return ActionCheck()
                return ActionFold()
