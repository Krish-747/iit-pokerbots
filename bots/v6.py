'''
alpha_q bot v6.0 — The Dynamic Auction & True Tracking Update

CRITICAL UPGRADES:
1. True Auction Tracking: We now log inferred bids when we LOSE the auction to remove downward bias.
2. Dynamic Bidding: Auction bids scale continuously based on hand strength, pot size, and opponent history.
3. Revealed Card Optimization: Monte Carlo leverages the revealed card as a blocker to catch impossible bluffs.
4. Anti-Maniac & Flop Floats: Dynamic call tolerances to catch donk bets and random floats to stop fit-or-fold.
'''
...
class Player(BaseBot):
    def __init__(self):
        ...
        self._our_last_bid     = 0    # Tracks our bid to analyze losses
    ...
    def _auction_bid(self, current_state):
        ...
        # Scale our counter-bid based on their historic average 
        if wr_base > 0.75:
            # Monster hand: Dynamic outbid ensuring we snipe the info
            base = int(pot * random.uniform(0.50, 0.80))
            bid_amount = max(base, int(avg_bid * 1.5))
        elif wr_base > 0.55:
            # Strong hand: Competitive randomized bid
            base = int(pot * random.uniform(0.25, 0.50))
            bid_amount = max(base, int(avg_bid * 1.1))
        elif wr_base > 0.40:
            # Marginal hand: Small pot fraction
            bid_amount = int(pot * random.uniform(0.10, 0.25))
        else:
            # Trash hand: Micro bid
            bid_amount = int(pot * random.uniform(0.0, 0.05))
            
        safety_cap = int(chips * 0.3)
        final_bid = max(1, min(bid_amount, safety_cap, chips-1, 4999))
        self._our_last_bid = final_bid  # Store for tracking
        return final_bid
    ...
    def get_move(self, game_info: GameInfo, current_state: PokerState):
        ...
        # --- TRUE AUCTION TRACKER FIX ---
        if street == 'flop' and self._auction_pre_pot > 0:
            if opp_known:
                # We WON the auction. The pot increased by their exact bid.
                delta = pot - self._auction_pre_pot
                if 0 < delta < 1000:
                    self.opp.record_opp_bid(int(delta))
            else:
                # We LOST the auction. They paid our bid, so the pot increased by our bid.
                # We mathematically know their true bid is higher than our bid.
                # Record a virtual bid to prevent our average from sinking!
        ...
        if street == 'pre-flop':
            ...
            if is_pair and hi >= 13:
                ...
                elif raise_level == 1 and can(ActionCall) and cost <= int(pot * call_tolerance):
                    return ActionCall()
                if can(ActionCheck): return ActionCheck()
                return ActionFold()
        ...
        if street in ['flop', 'turn', 'river']:
            ...
            if hclass in MONSTER:
                ...
                if street == 'flop' and can(ActionCall) and cost <= (pot * 0.50) and random.random() < 0.35:
                    return ActionCall()
                    
                if can(ActionRaise) and opp_r == 0 and cost == 0 and random.random() < 0.35:
                    self._street_raises[street] += 1
